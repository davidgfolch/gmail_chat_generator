...
class Processor:
    ...
    def run(self, start_date_str, end_date_str, label, comment=None):
        ...
        self._process_and_generate_report(
            all_messages_raw, start_date_str, end_date_str, label, comment
        )
    ...
    def _sync_cache(self, day_messages, current_ids, cache_filename, process_date_str):
        ...
        if missing_ids:
            print(
                f"Found {len(missing_ids)} new messages for {process_date_str}. Fetching details..."
            )
    ...
    def _process_and_generate_report(
        self, messages, start_date, end_date, label, comment=None
    ):
        try:
            print(f"Generating report: {start_date} to {end_date}, label: {label}")
            start_f = start_date.replace("-", "")
            end_f = end_date.replace("-", "")
            label_safe = label.replace("/", "-").replace("\\", "-")
            base_parts = [start_f, end_f, label_safe]
            if comment:
                comment_safe = comment.replace(" ", "_")
                base_parts.append(comment_safe)
            base_name = "-".join(base_parts)

            print(f"Group and sort: {len(messages)} messages...")
            sorted_threads = self.processor.process_threads(messages)
            print(f"Download and save images...")
            images_subdir = os.path.join("images", base_name)
            self._download_images(sorted_threads, images_subdir)
            print(f"Deduplicate content...")
            self.processor.deduplicate_thread_content(sorted_threads)
            print(f"Generate Report...")
            filename = os.path.join(config.OUTPUT_DIR, f"{base_name}.md")
            output_file = self.formatter.generate_report(
                start_date,
                end_date,
                label,
                sorted_threads,
            ...
            html_file = self.formatter.convert_to_html(output_file)
    ...
    def _download_images(self, threads, images_subdir):
        images_dir = os.path.join(config.OUTPUT_DIR, images_subdir)
        os.makedirs(images_dir, exist_ok=True)
        for thread_id, messages in threads:
            for content in messages:
                content["images"] = self._process_message_images(
                    content.get("images", []), images_dir, images_subdir
                )

    def _process_message_images(self, images, images_dir, images_rel_path):
        processed = []
        for img in images:
            safe_name = f"{img['message_id']}_{img['filename']}"
            img_path = os.path.join(images_dir, safe_name)
            if os.path.exists(img_path):
                img["path"] = os.path.join(images_rel_path, safe_name)
            ...
            if img.get("data"):
                ...
                img["path"] = os.path.join(images_rel_path, safe_name)
                processed.append(img)
            elif img.get("attachment_id") and self.client:
                attachment = self.client.get_attachment(
                    img["message_id"], img["attachment_id"]
                )
