...
class MessageBodyFormatter:
    ...
    def parse_citation_header(self, text):
        ...
        if match:
            ...
            formatted_date = f"{year}-{month}-{day.zfill(2)} {time}"
            ...
            return f"{colored_date} {colored_name}"
        return text
    ...
    def format_message_body(self, message_body):
        ...
        for i, line in enumerate(reflowed_lines):
            ...
            next_is_separator = (i < len(reflowed_lines) - 1 and self.is_separator_line(reflowed_lines[i + 1]))
            next_is_quote = (i < len(reflowed_lines) - 1 and reflowed_lines[i + 1].strip().startswith('>'))
            if is_quote:
                in_quote_block = self._handle_quote_line(line, is_separator, in_quote_block, formatted_parts)
            else:
                in_quote_block = self._handle_normal_line(line, is_separator, next_is_quote, next_is_separator, in_quote_block, formatted_parts, i, len(reflowed_lines))
    ...
    def _is_text(self, l):
        return l.strip() and not l.strip().startswith('>') and not self.is_separator_line(l)
    ...
    def _handle_normal_line(self, line, is_separator, next_is_quote, next_is_separator, in_quote_block, formatted_parts, i, total_lines):
        if in_quote_block:
            formatted_parts.append('\n')
            in_quote_block = False
        if line.strip():
    ...
    def _strip_result(self, result):
        while result.endswith('\n') or result.endswith('<br>') or result.strip().endswith('>'):
            if result.endswith('\n'):
                result = result[:-1]
