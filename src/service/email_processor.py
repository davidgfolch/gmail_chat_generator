...
class EmailProcessor:
    ...
    def _extract_content(self, message_resource):
        ...
        images = self._extract_images(payload, message_resource.get("id", ""))
    ...
    def _extract_images(self, payload, message_id):
        images = []
        if "parts" in payload:
            for part in payload["parts"]:
                if part["mimeType"].startswith("image/"):
                    ...
                    if "data" in body:
                        image_info["data"] = base64.urlsafe_b64decode(body["data"])
                    images.append(image_info)
                if "parts" in part:
                    images.extend(self._extract_images(part, message_id))
        return images
